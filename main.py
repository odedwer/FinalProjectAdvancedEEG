...
for s in subjects:
    ...
    # ica.apply(raw)
    # raw.save('PVI_EEGjunior_S{subject_number}_1-clean-raw.fif')
    # %%
    ...
    epochs_prime = mne.Epochs(raw, events, event_dict_reg, tmin=-.2, tmax=1.1, baseline=(-.2, -.05), reject={'eeg': 300e-6})
    # %% decoding
    ...
    epochs_prime = mne.Epochs(raw, events, event_dict, tmin=-.4, tmax=1.1, baseline=(-.4, -.1))#, reject={'eeg': 400e-6})
    event_dict = {"cue_R_reg": 112, "cue_L_reg": 111}
    epochs_cue = mne.Epochs(raw, events, event_dict, tmin=-.4, tmax=1.1, baseline=(-.4, -.1))#, reject={'eeg': 400e-6})
    # create classifier
    lda = LinearDiscriminantAnalysis()
    clf = make_pipeline(StandardScaler(), lda)
    ...
    scores_lc = mne.decoding.cross_val_multiscore(estimator=time_decoding, X=epochs_prime_lc.get_data('eeg'), y=labels_lc,cv=kfold)
    scores_lc = scores_lc.mean(axis=0)
    # %% right
    labels = epochs_prime.copy().events[:, 2]
    labels[lr_trials] = 0
    labels[rr_trials] = 1
    ...
    scores_rc = scores_rc.mean(axis=0)
    # plt.figure()
...
plt.hlines(0.5, epochs_prime.times.min(), epochs_prime.times.max(), linestyles=':', colors='k', zorder=7)
plt.title(f'Incongruent VS congruent trials decoding for left cues and right cues, average of all')
